<script>
    ...
    { // Creates new event
        ...
        $page = file_get_contents('wccw_new_1_master.php');
        ...
        $chkbox[$_SESSION['member_id']] = 'on';

        // Put the title and details into the form
        if (empty($title)) $title = "";
        $page = str_replace("##TITLE##", $title, $page);
        if (empty($details)) $details = "";
        $page = str_replace("##DETAILS##", $details, $page);

        // Show privacy status
        $page = str_replace("##PRIVATE_CHECKED##", (isset($private) ? "checked" : ""), $page);
        ...
        if (isset($invitee_group)) {
            ...
            // Now ensure that the proper radio button is still checked when resending the page
            $page = selectability($invitee_group, $page);
        } else $where = 'FALSE';
        $where .= " OR member_id IN (" . implode(',', array_keys($chkbox)) . ')';
        ...
        if (empty($chkbox)) $chkbox = array(null);
        $query = "SELECT 
            member_id, userid, edit_privilege, dbc, 
            concat(first_name,' ',last_name) AS full_name, 
            family_ptr, 
            member_id IN (" . implode(',', array_keys($chkbox)) . ")  AS checked
        ...
        foreach ($candidates as $c => $candidate) {
            ...
            if (isset($chkbox[$c])) $clist .= ' checked ';
        ...
        if (isset($dates)) $page = str_replace("placeholder='Click here to select dates'>", ">$dates", $page);
